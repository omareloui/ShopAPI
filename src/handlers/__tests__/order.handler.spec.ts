...
import { CreateOrder, Order, PopulatedOrder, Product, UpdateOrder, User } from "../../@types";
...
describe("Order Handler", () => {
  ...
  describe("Basic functionality", () => {
    ...
    it("should update the order on PUT /orders/:id", async () => {
      const order = generateOrder()
      const { body: createdOrder } = await request.post("/orders").send(order)
      const { body: resOrder } = await request.put(`/orders/${createdOrder.id}`).send({ quantity: 899 } as UpdateOrder)
